...
class PNTS 
 ...
 // CREATE
  ...
  void placeWxWpointsOnHexGridTrimToDisk(int w, float r) // make a 2D grid of w x w vertices
   {
   PNT P;
   empty();
   float dx = .7*height/(w-1);
   ...
   for (int i=0; i<w/h; i++) 
     for (int j=0; j<w; j++)
       ...
       if(i%2==0) P=P(dx*j+.1*height,dy*i+.1*height);
       else  P=P(dx*j+dx/2+.1*height,dy*i+.1*height);
       if(d(ScreenCenter(),P)<r) addPoint(P);
       }
   }    
