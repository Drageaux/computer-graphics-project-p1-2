...
void setup()               // executed once at the beginning 
  ...
  //Sites.placeWxWpointsOnHexGridTrimToDisk(10,ringRadius); // makes initial hexgrid of sites
  Sites.loadControlPointsFromFile("data/pts");  // loads points form file saved with this program
   Sites.placeWxWpointsOnHexGridTrimToDisk(10,ringRadius);
...
void draw()      // executed at each frame
  ...
  if(scribeText)
    {
    if(live) scribeFooter("LIVE",3);
    f(black); scribeFooter("time = "+nf(currentTime,0,2),4);
    }
